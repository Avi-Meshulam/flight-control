...
+ Flights precedence is determined based on waiting time. The longer a flight waits, the higher priority it gets. If two flights has the same waiting time, priority is given to flights in landing leg (leg no. 3), then to departing flights and otherwise to one of them arbitrarily.
...
+ Both client and simulator save their data in DB and therefore can always recover from unexpected shutdown. The tower saves the state of the airport legs (leg-state and occupying flight, if any), the arriving/departing/emergency queues and the user settings. The simulator saves flights data, while deleting a flight upon getting a response from tower. If the database is not reachable, the tower stops handling requests, since it must constantly update its data in order to be able to recover (technically, it is possible to temporarily save the data to a local file and then sync with the database). When the simulator exhausts its data, it refills its lists with the same data again, so it can keep transmitting infinitivally.
